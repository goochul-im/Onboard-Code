import type { SourceLanguage } from "../core";
...
const PYTHON_TYPES = new Set(["bool", "bytes", "dict", "float", "int", "list", "object", "set", "str", "tuple", "type"]);
...
const PHP_TYPES = new Set(["bool", "float", "int", "iterable", "mixed", "never", "object", "self", "string", "void"]);
...
export function detectSourceLanguage(relativePath: string): SourceLanguage {
  ...
  if (relativePath.endsWith(".php")) return "php";
...
function tokenizeLine(line: string, language: SourceLanguage, state: HighlightState): SyntaxToken[] {
  ...
  while (index < line.length) {
    ...
    const pythonPrefix = language === "python" ? /^[rRuUbBfF]{0,2}(?:'''|"""|'|")/.exec(remaining) : null;
    ...
    if (quote) {
      ...
      if (end < 0) {
        ...
        if (isTripleQuote) state.tripleQuote = delimiter as "'''" | "\"\"\"";
        return tokens;
      }
...
function findStringEnd(line: string, index: number, delimiter: string): number {
  for (let cursor = index; cursor < line.length; cursor += 1) {
    ...
    if (line.startsWith(delimiter, cursor)) return cursor;
  }
  return -1;
}

function classifyWord(word: string, line: string, index: number, language: SourceLanguage): SyntaxTokenKind {
  const keywords = language === "java" ? JAVA_KEYWORDS : language === "php" ? PHP_KEYWORDS : language === "python" ? PYTHON_KEYWORDS : TYPESCRIPT_KEYWORDS;
  const types = language === "java" ? JAVA_TYPES : language === "php" ? PHP_TYPES : language === "python" ? PYTHON_TYPES : TYPESCRIPT_TYPES;
