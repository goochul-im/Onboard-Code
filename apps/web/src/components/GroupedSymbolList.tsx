...
export function GroupedSymbolList({
  ...
  useEffect(() => {
    ...
    const selectedGroup = groups.find((group) => group.symbols.some((symbol) => symbol.id === selectedSymbolId));
    const queryStarted = query.trim().length > 0;
    const queryCleared = previousQuery.current.trim().length > 0 && !queryStarted;
    previousQuery.current = query;

    setExpandedGroups((current) => {
      if (queryStarted) return groupKeys;
      if (queryCleared) return new Set(selectedGroup ? [selectedGroup.key] : []);
  ...
  if (groups.length === 0) {
    return showEmptyMessage ? <p className="muted">분석 후 함수를 검색할 수 있습니다.</p> : null;
  }
          ...
          <details
            ...
            onToggle={(event) => {
              ...
              setExpandedGroups((current) => {
                ...
                if (expanded) next.add(group.key);
