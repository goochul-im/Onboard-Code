...
export function placeGraphNodeAction(
  ...
  if (node.x2 <= 0 || node.x1 >= containerWidth || node.y2 <= 0 || node.y1 >= containerHeight) return null;
  const centeredTop = clamp((node.y1 + node.y2 - actionHeight) / 2, EDGE_PADDING, containerHeight - actionHeight - EDGE_PADDING);
  ...
  return {
    left: clamp((node.x1 + node.x2 - actionWidth) / 2, EDGE_PADDING, containerWidth - actionWidth - EDGE_PADDING),
    top: clamp(node.y2 + NODE_GAP, EDGE_PADDING, containerHeight - actionHeight - EDGE_PADDING),
    placement: "below",
  };
}
