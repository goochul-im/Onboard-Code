...
3. 이후 커밋이나 작업 트리가 바뀌면 **변경된 코드 확인하기**를 눌러 마지막 성공 분석과 현재 소스를 비교합니다. 버튼의 `?` 도움말에서 비교 범위와 결과 해석 방법을 확인할 수 있으며, 직접 변경된 함수와 최대 3단계의 caller 확인 후보를 볼 수 있습니다.
4. 왼쪽에서 클래스·파일 그룹을 펼치거나 함수를 검색하고, 중앙 그래프에서 caller/callee를 1~3단계로 펼칩니다.
5. Record에서 코드를 일반 드래그해 복사하거나, macOS에서는 `Command`, Windows에서는 `Ctrl`을 누른 채 클릭·드래그해 줄 참조를 노트에 넣습니다. 문서의 `[line:31]` 참조를 클릭하면 해당 코드로 돌아갈 수 있습니다.
6. Explore의 **컬렉션에 추가**로 여러 함수를 기능별 읽기 순서로 묶습니다. Collections에서 역할·메모·기능 개요를 작성하고, 작성 순서와 컬렉션 내부의 실제 호출 관계를 번갈아 확인할 수 있습니다.
7. **Confluence용 복사**를 누르면 줄 참조가 실제 코드 블록으로 치환된 함수 문서나 컬렉션을 Confluence Cloud 편집기에 붙여넣을 수 있습니다.
...
변경 영향은 정적 호출 관계를 이용한 검토 후보이며 런타임 영향을 확정하지 않습니다. 기준 분석 이후의 커밋, staged·unstaged 변경과 추적되지 않은 지원 소스 파일을 포함하고, 코드 분석을 다시 실행하기 전에는 현재 인덱스를 변경하지 않습니다.
...
- 기능별 컬렉션에는 개발자가 정한 함수 순서·역할·메모·개요가 저장되며, 기존 함수 노트는 복제하지 않고 연결해 사용합니다.
- 파일 이동 뒤 FQN과 signature가 일치하면 노트를 새 함수로 다시 연결합니다.
- 함수 이름이 바뀌거나 삭제되어 안전한 자동 연결이 불가능하면 노트를 삭제하지 않고 `연결 필요 노트`로 보관합니다.
- 컬렉션 함수도 재분석 뒤 안전하게 다시 연결하며, 찾지 못한 항목은 삭제하지 않고 `연결 필요` 상태로 남깁니다.
